...
<%	
menulist = "1=General,2=Distribution,3=Programs,4=Advertisements,5=Notes"
...
Select Case KeySet
	...
	Case "4"
		...
		query = "SELECT DISTINCT dbo.v_Advertisement.AdvertisementID, " & _
			"dbo.v_Advertisement.AdvertisementName, dbo.v_Advertisement.PackageID, " & _
			"dbo.v_Package.Name AS PackageName, dbo.v_Advertisement.ProgramName, " & _
            "dbo.v_Advertisement.CollectionID, dbo.v_Collection.Name AS CollectionName, dbo.v_Collection.MemberCount " & _
			"FROM dbo.v_Advertisement INNER JOIN " & _
            "dbo.v_Package ON dbo.v_Advertisement.PackageID = dbo.v_Package.PackageID INNER JOIN " & _
            "dbo.v_Collection ON dbo.v_Advertisement.CollectionID = dbo.v_Collection.CollectionID " & _
			"WHERE (dbo.v_Advertisement.PackageID='" & RecID & "') " & _
			"ORDER BY " & SortBy
		CMWT_DB_QUERY Application("DSN_CMDB"), query
		CMWT_DB_TABLEGRID rs, "", "package.asp?ks=3", ""
		CMWT_DB_CLOSE()
